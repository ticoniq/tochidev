...
function Navbar({ dark, data}) {
  ...
  return (
    <div className={`bg-new fixed w-full top-0 z-20 bg-white dark:bg-base-100 dark:text-white transition-all duration-200 ${showShadow ? 'shadow-md' : ''}`} id="Home">
      <header className="inset-x-0 top-0 z-50 dark:bg-base-100">
        <nav className="flex items-center justify-between py-4 max-w-7xl mx-auto px-5" aria-label="Global">
          <div className="flex lg:flex-1">
            <a href="#Home" className="">
              <span className="font-bold text-xl">TochiDev</span>
            </a>
          </div>
        ...
        <Dialog as="div" className="lg:hidden" open={mobileMenuOpen} onClose={setMobileMenuOpen}>
          ...
          <Dialog.Panel className="fixed inset-y-0 right-0 z-50 w-full h-96 overflow-auto bg-white px-6 py-6 sm:max-w-full dark:bg-base-100">
            <div className="flex items-center justify-between">
              <a href="/" className="font-bold text-xl dark:text-white">TochiDev</a>
              <button
                type="button"
